...
{
	hoedown_html_renderer_state *state = data->opaque;
	hoedown_buffer_puts(ob, USE_XHTML(state) ? "<br/>" : "<br>");
	return 1;
}
...
hoedown_renderer *
hoedown_html_renderer_new(hoedown_html_flags render_flags, int nesting_level, uint8_t hasStrong)
{
	static const hoedown_renderer cb_default = {
		NULL,
	...
	//Penner:support not have strong tag
	if (!hasStrong) {
		renderer->emphasis = NULL;
		renderer->double_emphasis = NULL;
		renderer->triple_emphasis = NULL;
	}
